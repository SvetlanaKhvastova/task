...
const style = /*html*/ `
            <style>
                ...
                .css-1fdnw4t .selectInline .selectTitle {margin: 0 !important;}
                .tooltip_wrapper {
                    position: absolute;
                    top: 0;
                }
                ...
                .css-1fdnw4t .selectInline {width: 48% !important;}

                .full_width .selectInline {width: 100% !important;}
...
const addTooltips = () => {
  ...
  const tooltipSelect = [
    // Credit Score, Liability Coverage, C. and C. Insurance
  ...
  tooltipSelect.forEach((select, index) =>
...
const hideSelects = () => {
  const secondPartSelects = [queryAll(".inlineSelectContainer")[2], queryAll(".inlineSelectContainer")[3], queryAll(".inlineSelectContainer")[4], query(".add-student")]

  secondPartSelects.forEach((select) => {
    select.hidden = true
  })
  secondPartSelects[0].insertAdjacentHTML(
    "beforebegin",
    `
  ...
  const waitForBtn = setInterval(() => {
    if (query(".continue") && query(".back")) {
      ...
      const firstPartSelects = [queryAll(".inlineSelectContainer")[0], queryAll(".inlineSelectContainer")[1], query(".continue")]
      ...
      const showSecondPart = () => {
        ...
        firstPartSelects.forEach((select) => {
          select.hidden = true
        })
...
const waitForCalc = setInterval(() => {
  if (query(".e1ssirya5 h4")) {
    ...
    query(".e1ssirya5 h4").insertAdjacentHTML(
      "afterend",
      `
    ...
    const waitForFrSelectWrapper = setInterval(() => {
      if (query(".fr_select_wrapper .selectInline") && query(".state") && query("div.selectInline.last")) {
  ...
  for (let mutation of mutations) {
    for (let node of mutation.addedNodes) {
      ...
      if (node.matches(".css-26l3qy-menu")) {
        ...
        fireEvent("Click on:", `List ${select} opened`)
        ...
        query(".css-26l3qy-menu > div").addEventListener("click", (e) => {
          ...
          fireEvent("Click on", `List ${selectName} item ${selectValue} selected`)
        })
      }
