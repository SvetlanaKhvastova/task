...
class AdultsVersion {
  ...
  init() {
    ...
    clarityInterval('exp_adilt', 'variant_1')
  ...
  renderNewRedirectLink() {
    waitForElement('a.learn-now').then(i => {
      if (!$el('.new_redirect_link')) {
        $el('.learn-now').insertAdjacentHTML('afterend', newRedirectLinkVerFirst)
      }
    })
  }
  ...
  clickLinkHandler() {
    waitForElement('.new_redirect_link').then(i => {
      $el('.new_redirect_link a').addEventListener('click', e => {
        pushData('exp_adult_button_01', 'See the grownup version', 'Click', 'First screen')
      })
    })
  }
