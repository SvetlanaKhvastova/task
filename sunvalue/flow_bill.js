(function () {
  console.log("%c EXP: Introduce value proposition and increase engagement during the flow (DEV: Olha)", "background: #3498eb; color: #fccf3a; font-size: 20px; font-weight: bold;");
  ...
  class changeFlow {
    ...
    init() {
      ...
      const globalMutation = new MutationObserver((mutations) => {
        ...
        if (this.checkPageUrl() === "flow-bill") {
          if ($(".swiper-slide").eq(1).hasClass("swiper-slide-active") || $(".swiper-slide").eq(2).hasClass("swiper-slide-active") || $(".swiper-slide").eq(3).hasClass("swiper-slide-active") || $(".swiper-slide").eq(6).hasClass("swiper-slide-active")) {
        ...
        if (device == "desktop") {
          if ($(".swiper-wrapper .swiper-slide").eq(5).hasClass("swiper-slide-active") && clickCount == 0) {
            clickCount = 1;
            $(".swiper-wrapper .swiper-slide").eq(5).find(".input-error").removeClass("input-error");
    ...
    changeSlides() {
      let city = $("title").text().split("City")[0].split("Solar")[0].split(":")[0].replace("Enter", "").replace("Zip", "").trim();
      ...
      $(".swiper-wrapper .swiper-slide").each(function (index, element) {
        let _this = $(element);
        if (index == 0) {
          _this.find("h2").html(`${city} Solar Incentive Program<span>How much is your latest monthly energy bill?</span>`);
        } else if (index == 1) {
          _this.find(".pac-target-input").before(dataIcons.location);
          _this.find(".pac-target-input").attr("placeholder", "Enter your property address ");
        } else if (index == 2) {
          _this.find(".container").prepend(`<h4 class="sub-title banner sub-headline">✅  Incentives are found in your area!</h4>`);
        } else if (index == 4) {
          _this.find(".container").prepend(`<h4 class="sub-title banner sub-headline">✅  You're almost qualified for this limited-time program.</h4>`);
          ...
          _this.find(".small-container").after(`<p class="crs_text_s">${dataIcons.chieldCheck} Your data is safe and secure.</p>`);
        }
      });
      ...
      if (device === "desktop") {
        $("#slider-block").append(`<button type="button" class="btn crs_next">Next</button>`);
        ...
        $(".final-btn").prepend(`<button type="button" class="btn crs_back">Back</button>`);
      ...
      $(".back-link").click(function () {
        ...
        step = step.includes("2") ? "Second" : step.includes("3") ? "Third" : step.includes("4") ? "Fourth" : step.includes("5") ? "Fifth" : step.includes("6") ? "Sixth" : step.includes("7") ? "Seventh" : "First";

        let title = step + " step " + $(".swiper-slide-next .title").eq(1).text();

        let eventName = step.includes("Second") ? "secstlocat" : step.includes("Third") ? "thriprovid" : step.includes("Fourth") ? "foursunli" : step.includes("Fifth") ? "fiftstran" : step.includes("Sixth") ? "sixaddres" : "sevenumber";

        pushDataLayer(`exp_val_incr_eng_but_${eventName}_back`, "Back", "Button", title);
      });

      $("#estimate-shade .custom-radio-item input").change(function () {
        let eventName = $(this).parent().text().includes("Some") ? "some" : $(this).parent().text().includes("Sunlight") ? "full" : $(this).parent().text().includes("Severe") ? "severe" : "uncer";
