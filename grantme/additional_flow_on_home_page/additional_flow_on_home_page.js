console.log("%c EXP: additional_flow_on_home_page (DEV: SKh)", "background: #3498eb; color: #fccf3a; font-size: 20px; font-weight: bold;");
...
class HomePage {
  ...
  reDesignHeader() {
    ...
    if (this.device === "Mobile") {
      ...
      selectorTagHeader = ".elementor-24572 .elementor-element.elementor-element-1e45e76.elementor-column.elementor-element[data-element_type=column]>.elementor-widget-wrap.elementor-element-populated";
  ...
  initStickyHeader() {
    ...
    const stickyHeader = /* HTML */ `
      ...
      <div class="sticky_header">
        <div class="container">${this.logoHeaderHtml()} ${this.trustScoreStarsBlockHtml()} ${this.scheduleFreeConsultationBtnHtml()}</div>
      </div>
    `;
      ...
      <div id="heroScheduleConsultationCallBlock">
        ...
        <ul>
          <li class="schedule_a_free_block">
            ...
            <p class="schedule_txt deck_var">Discuss your (your child) background and interests with one of our expert program advisors—they’ll help you figure out if a GrantMe education program is right for you.</p>
            <p class="schedule_txt mob_var">
              In a quick <b>15-minute call</b> with one of our expert advisors, explore whether a GrantMe education program suits you. <br />
  ...
  reDesignTuitionSection() {
    waitForElement(".tuition_section a.btn").then((el) => {
      ...
      const newTuitionTxt = "Unlock university admission and funding opportunities available to students. Click the button below to book a call now!";
...
class BookingPage {
    ...
    const titleHtml = /* HTML */ `
      <style>
        ...
        .new_hero_title span:before {
          position: absolute;
          content: "";
          background: url(/themes/custom/grantmenew2/css/features/../../images/booking/right-arrow-yellow.svg) no-repeat center center;
          width: 24px;
          height: 24px;
          top: 50%;
          right: -50px;
  ...
  onChangeTxtFooterBtn() {
    waitForElement(".footer-start-free-trial a.btn.primary-solid-yellow").then((el) => {
      ...
      el.addEventListener("click", (e) => {
        e.preventDefault();
        pushDataLayer("exp_add_flow_but_bookcons_reque", "Request Consultation", "Button", "Booking page  Schedule a Complimentary Consultation");
      ...
      $$el(".book_free_call_btn")?.forEach((el) => {
        el.addEventListener("click", (e) => {
          ...
          if (e.target.closest(".is_fixed") || e.target.closest(".sticky_box")) {
            pushDataLayer("exp_add_flow_sticbut_bookcons_bookcall", "Book a Free Call", "Sticky button ", "Booking page  Schedule a Complimentary Consultation");
          }
          let coverageElem = $el("#newScheduleBox");
          checkScrollPosition(0, coverageElem);
        });
      ...
      $$el(".slick-arrow").forEach((el) => {
        el.addEventListener("click", function (e) {
          if (e.currentTarget.classList.contains("testimonials_arrow_prev")) {
            pushDataLayer("exp_add_flow_but_bookcons_arrow", "left", "Button", "Booking page  Schedule a Complimentary Consultation");
          } else {
            pushDataLayer("exp_add_flow_but_bookcons_arrow", "right", "Button", "Booking page  Schedule a Complimentary Consultation");
          }
        });
      });
    });
    ...
    waitForElement(".testimonials_video").then((el) => {
      $$el(".wistia_responsive_padding").forEach((el) => {
        el.addEventListener("click", () => {
          pushDataLayer("exp_add_flow_clic_bookcons_video", "Video", "Click", "Booking page Schedule a Complimentary Consultation");
        });
      });
    });
  }
    ...
    waitForElement("#accordion").then((el) => {
      $$el("#accordion .panel .panel-heading a").forEach((el) => {
        el.addEventListener("click", (link) => {
          pushDataLayer("exp_add_flow_accor_bookcons_quest", `${link.currentTarget.textContent} - Questions`, "Accordion", "Booking page  Schedule a Complimentary Consultation");
        });
      });
    });
  }
    ...
    const mainStyles = /* HTML */ `
      <style>
        ...
        @media (max-width: 768px) {
          ...
          .our_consultants_wrapper > p:before {
            background: url(/themes/custom/grantmenew2/css/features/../../images/booking/telephone-yellow.svg) no-repeat center center;
          }
          body .consultation_descr_box > div h3 {
