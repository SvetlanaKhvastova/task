    ...
    if (el.innerText.toLowerCase().includes(`colour`)) {
      ...
      function addSpanColor() {
        document.querySelectorAll(".variants.form .square").forEach((i) => {
          ...
          document.querySelector(".check-color").insertAdjacentHTML("beforeend", ` <button type="button" class="controls-btn colors-btn ${active} ${dizabledBtn}" style='background: ${bg}'></button>`);
    ...
    if (el.innerText.toLowerCase().includes(`size`)) {
      ...
      function addSpanSize() {
        btnSize.forEach((i) => {
          ...
          document.querySelector(".check-size").insertAdjacentHTML("beforeend", `<button type="button" class="size-variant size-variant-btn ${dizabledBackground} ${active}">${textContent}</button>`);
