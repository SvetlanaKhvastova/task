(() => {
  ...
  const icons = {
    arrowDown: /* HTML */ `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="17" viewBox="0 0 16 17" fill="none">
      <g clip-path="url(#clip0_3086_590)">
        <path d="M4.8514 7.0576L7.99785 10.204L11.1443 7.0576C11.1906 7.01071 11.2458 6.97344 11.3066 6.94794C11.3673 6.92243 11.4326 6.9092 11.4985 6.909C11.5644 6.9088 11.6297 6.92164 11.6906 6.94677C11.7516 6.9719 11.8069 7.00884 11.8536 7.05545C11.9002 7.10206 11.9371 7.15742 11.9622 7.21836C11.9874 7.2793 12.0002 7.3446 12 7.41052C11.9998 7.47643 11.9866 7.54166 11.9611 7.60244C11.9356 7.66323 11.8983 7.71837 11.8514 7.76469L8.3514 11.2647C8.25763 11.3585 8.13046 11.4111 7.99785 11.4111C7.86525 11.4111 7.73807 11.3585 7.64431 11.2647L4.14431 7.76469C4.05148 7.67073 3.9996 7.54386 4 7.41177C4.0004 7.27969 4.05305 7.15313 4.14645 7.05974C4.23984 6.96634 4.3664 6.9137 4.49848 6.91329C4.63056 6.91289 4.75744 6.96477 4.8514 7.0576Z" fill="#212121" />
      </g>
      <defs>
                    ...
                    <div class="swiper-slide">
                      <a class="swipe-item" href="${product.url}">
                        <div class="wishlist-wrap" data-product-id="${product.id}">
                          <action cy-wishlistaddbtn="" class="wishlist-button cursor-pointer" _nghost-ng-c3660662962=""
                            ...
                            ><!----><span _ngcontent-ng-c3660662962="" class="button__body"><i aria-hidden="true" class="inline-flex icon-wishlist" style="font-size: 1.1em;"></i></span
                          ></action>
                        </div>
                        ...
                        <p>${product.price}</p>
                        ${product.reviews ? `<rating class="inline-flex fs-7-x fs-7-l ng-star-inserted">${stars} (${product.reviews.count})</rating>` : ""}
                      </a>
                    </div>
                  `;
                })
        ...
        item.addEventListener("click", () => {
          waitForElement("product-reviews-modal").then(() => {
            $el("._sortLabel").addEventListener("click", () => {
              pushDataLayer("exp_impro_pdp_dropd_customer_sort", "Sort by", "Dropdown", '"PDP Rated as ‘excellent’ by our customers"');
            });
              ...
              $$el("button").forEach((btn) => {
                btn.addEventListener("click", () => {
                  if (btn.textContent.includes("Show more")) {
                    pushDataLayer("exp_impro_pdp_review_customer_read", "Read more", "Review", "PDP Rated as ‘excellent’ by our customers");
                  }
                });
              });
            });
