console.log("%c EXP: Exit intent popup (DEV: Olha)", "background: #3498eb; color: #fccf3a; font-size: 20px; font-weight: bold;");
...
function copyText(target) {
  ...
  setTimeout(() => {
    inputElement.nextElementSibling.querySelector("span").innerText = inputElement.value;
  }, 2000);
...
function isUrlValid(value) {
  let res = value.match(/[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);
...
function updatePeopleCountInView(parent) {
  ...
  const itemsInView = listItems.filter((item) => isElementInView(item, popupList));
...
class ExitIntentPopup {
  ...
  init() {
    ...
    const globalMutation = new MutationObserver((mutations) => {
      if (this.checkPageUrl() === "basket" || this.checkPageUrl() === "checkout") {
  ...
  showPopup() {
    ...
    if ($el(`.crs_popup .crs_list`)) {
      $el(`.crs_popup .crs_list`).scrollLeft = this.itemsLengthBasket > 2 ? 122 : ($el(".crs_list").scrollWidth - $el(".crs_list").clientWidth) / 2;
    }
    ...
    if (this.itemsLengthBasket == 0) {
      if (!sessionStorage.getItem("popupShownB") && $el(`.crs_popup[data-query="b"]`)) {
      ...
      } else if (!sessionStorage.getItem("popupShownD") && $el(`.crs_popup[data-query="d"]`)) {
      ...
      if (!sessionStorage.getItem("popupShownA") && $el(`.crs_popup[data-query="a"]`)) {
        sessionStorage.setItem("popupShownA", "true");
        $el(`.crs_popup[data-query="a"]`).classList.add("active");
      } else if (!sessionStorage.getItem("popupShownC") && $el(`.crs_popup[data-query="c"]`)) {
  ...
  actionExitIntentPopup() {
    ...
    let scrollSpeed = () => {
      ...
      if (currentSpeed > speedValue && window.scrollY != 0 && this.intervalBetweenPopup == 0) {
  ...
  actionPopup(selector) {
    ...
    let locEvent = title.includes("Would you like") ? "Welcome to LeMieux" : title.includes("almost yours") ? "It’s almost yours!" : title.includes("Autumn/winter") ? "Autumn/winter 2023most wanted" : "Check out now and get 10% off your first order";
    ...
    selector.querySelector(".crs_popup_close").addEventListener("click", () => {
      ...
      pushDataLayer([`exp_exi_int_popup_but_${this.nameCountEvent + nameEvent}_clos`, "Close", "Button", this.locCountEvent + locEvent]);
      pushDataLayer([`exp_exi_int_popup_vis_${this.nameCountEvent + nameEvent}_focu`, startTime, "Visibility", this.locCountEvent + locEvent]);
    });
    ...
    if (selector.getAttribute("data-query") == "d") {
      selector.querySelectorAll(".crs_popup_complete").forEach((item) => {
        item.addEventListener("click", (e) => {
          pushDataLayer([`exp_exi_int_popup_but_autwint_kit`, `${item.closest(".col-md-4").querySelector("p").innerText} - Shop Now`, "Button", "Popup Autumn/winter 2023 most wanted"]);

          clearInterval(startTimeInterval);

          pushDataLayer([`exp_exi_int_popup_vis_autwint_focu`, startTime, "Visibility", this.locCountEvent + locEvent]);
        });
      });
    ...
    selector.querySelector(".crs_popup_complete").addEventListener("click", (e) => {
      e.preventDefault();

      let nameBtn = e.target.innerText.includes("Complete") ? "compl" : e.target.innerText.includes("Sign up") ? "sign" : "check";

      pushDataLayer([`exp_exi_int_popup_but_${this.nameCountEvent + nameEvent}_${nameBtn}`, e.target.innerText, "Button", this.locCountEvent + locEvent]);

      if (selector.getAttribute("data-query") == "b") {
        let value = selector.querySelector("input").value;

        if (value != "" && isUrlValid(value)) {
          selector.querySelector("input").parentElement.classList.remove("crs_error");

          let waitFoundBtnKlaviyo = setInterval(() => {
            if ($el('.needsclick.kl-private-reset-css-Xuajs1 [type="email"]')) {
              clearInterval(waitFoundBtnKlaviyo);

              const inputElement = $el('.needsclick.kl-private-reset-css-Xuajs1 input[type="email"]');
              inputElement.value = value;
              inputElement.dispatchEvent(new Event("input", { bubbles: true }));

              $el('.needsclick.kl-private-reset-css-Xuajs1 [data-testid="form-component"] button').click();

              let waitGetDiscount = setInterval(() => {
                if ($el(".needsclick.kl-private-reset-css-Xuajs1") && $el("form.needsclick > div > div > div > div > div > div") && ($el(".needsclick.kl-private-reset-css-Xuajs1").innerText.toUpperCase().includes("LMWELCOME10") || $el(".needsclick.kl-private-reset-css-Xuajs1").innerText.toUpperCase().includes("FIRSTORDERFREE"))) {
                  ...
                  $el("form.needsclick > div > div > div > div > div > div").addEventListener("click", (e) => {
                    pushDataLayer([`exp_use_this_popup_copy_button`, "Copy", "Button", "Popup Use this code"]);
                  });
                  ...
                  let waitCloseDiscount = setInterval(() => {
                    if (!$el(".needsclick.kl-private-reset-css-Xuajs1")) {
                      ...
                      pushDataLayer([`exp_use_this_popup_vis_focus`, startTime, "Visibility", "Popup Use this code"]);
                    }
                  }, 100);
          ...
          selector.querySelector("input").parentElement.classList.add("crs_error");
        }
          ...
          <div class="container flex">
              ...
              <div class="crs_popup_content">
                <h2 class="text-center">
                  ${this.idKlaviyoForm == "RQWev3" ? `Check out now and get <br><span class="crs_c-red">free delivery</span>  on your first order` : `Check out now and get <br><span class="crs_c-red">10% off</span> your first order`}
                </h2>
                ...
                <div class="crs_list_1">
                  ...
                  <div class="crs_popup_info flex flex-middle">
                    ${dataIcons.peopleIcon}
                      <p> Popular product. <b>${this.getRandomNumber(1, 10)} people</b> are currently looking at it.</p>
                  </div>
                </div>
                <div class="crs_code">
      ...
      <div class="crs_popup" data-query="a">
          <div class="container flex">
              ...
              <div class="crs_popup_info flex items-center">
                ${dataIcons.peopleIcon}
                  <p>There are <b>${this.getRandomNumber(4, 15)} people</b> looking at this product. We can’t guarantee its availability if you don't complete the purchase now</p>
              </div>
  ...
  addPopupWelcomeTo() {
    ...
    let waitOneMinutes = setInterval(() => {
      if (this.intervalBetweenPopup == 0) {
        ...
        setTimeout(() => {
          if (!sessionStorage.getItem("popupShownB") && this.itemsLengthBasket == 0) {
            sessionStorage.setItem("popupShownB", "true");
  ...
  updateProductsInPopup() {
    ...
    getFetch("p/customer/data").then((data) => {
      ...
      if (this.itemsLengthBasket == 1) {
        ...
        $el(`.crs_popup .crs_list`).parentElement.classList.remove("crs_list_1");
      }

      if (this.itemsLengthBasket > 0) {
        for (let i = 0; i < this.itemsLengthBasket; i++) {
          getFetch(`n/product/${itemsBasket[i].product}/verbosity/3`).then((dataItem) => {
            ...
            if (isBundle == "") {
              if (item.size) {
                size = JSON.stringify(window.autoInitData.data.attribute).split(`${item.size},"label":"`)[1].split('"')[0];
                size = size.split("\\") ? size.replace("\\", '"') : size;
              }
              if (item.color) {
                color = JSON.stringify(window.autoInitData.data.attribute).split(`${item.color},"label":"`)[1].split('"')[0];
              }
              options = `${size} | ${color}`;
            } else {
              options = isBundle;
            ...
            let from = $el(`.crs_popup`).getAttribute("data-query") == "a" ? 4 : 1;
            let to = $el(`.crs_popup`).getAttribute("data-query") == "a" ? 15 : 10;
            $el(`.crs_popup .crs_list`).insertAdjacentHTML(
              "beforeend",
              `<li data-id="${itemsBasket[i].id}" data-product="${itemsBasket[i].product}" data-people="${this.getRandomNumber(from, to)}">
                  <img src="/static/media/catalog${item.image}" alt="${item.meta.title}">
                  <div>
                      <p class="crs_popup_name"><b>${item.meta.title}</b></p>
                      <p class="crs_popup_span">${options}</p>
                      <p class="crs_popup_price"><b>${this.currency + itemsBasket[i].price}</b></p>
            ...
            if (i >= this.itemsLengthBasket - 1 && $el('.crs_popup[data-query="c"] .swiper-wrapper')) {
              $el('.crs_popup[data-query="c"] .swiper-wrapper').innerHTML = slide;
              initSwiper();
            }
          });
        }
