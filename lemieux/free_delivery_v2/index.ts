...
class FreeDelivery {
  ...
  init() {
    waitForElement('body.content-initiated').then(() => {
      ...
      document.head.insertAdjacentHTML('afterbegin', `<style>${styleMain}</style>`)
    })
  }
      ...
      case 'product':
        ...
        if (!$el('header .free_delivery_banner')) {
          $el('#header > .page-header__inner')?.insertAdjacentHTML(
            place,
            freeDeliveryBanner(this.country, this.currency, this.threshold)
          )
          if ($el('header .free_delivery_banner')) {
  ...
  changeIconHeader(): void {
    ...
    $$el('basket-qty').forEach(el => {
      ...
      if (thresholdExceeded) {
        ...
        el.classList.add('free_delivery_icon')
        lastHeaderIcon.style.opacity = '0'
      } else {
        ...
        lastHeaderIcon.style.opacity = '1'
        if (el.classList.contains('free_delivery_icon')) {
          el.classList.remove('free_delivery_icon')
        }
        let s = setInterval(() => {
          if (!$el('.cdk-overlay-backdrop')) {
            clearInterval(s)
