(function () {
  console.log("%c EXP: AOV hypothesis (DEV: Olha)", "background: #3498eb; color: #fccf3a; font-size: 20px; font-weight: bold;");
  ...
  function pushDataLayer(event_name, event_desc, event_type, event_loc) {
    ...
    console.dir(event_name + " / " + event_desc + " / " + event_type + " / " + event_loc);
  }
  ...
  class UpgradePlan {
    ...
    setDataCollections() {
      waitForElement(".ProductForm__AddToCart").then((el) => {
        ...
        el.addEventListener("click", () => {
          ...
          const idPDP = $el(".Product__InfoWrap form").getAttribute("id").split("product_form_")[1];
          ...
          $$el(".ProductForm__Variants li").forEach((item, index) => {
            ...
            this.dataCollections[idPDP].push({
              priceGB: (+extractNumber(item.querySelector(".pdp-cmp-price").innerText)).toFixed(2), //pdp-cmp-price
              priceOneGB: (+extractNumber(item.querySelector(".Pro-varint-price-inner").innerText)).toFixed(2),
              ...
              productId: idPDP,
              option: input.value,
              currency: extractCurrency(item.querySelector(".Pro-varint-price-inner").innerHTML.split("/")[0]),
              image: $el(`.Product__Gallery .Product__SlideItem[data-media-id="${input.dataset.mediaId}"] img`).srcset,
    ...
    upsellBlockHtml(data, dataUpgrade, dataItem, index) {
      ...
      if (data.length > 1) {
        text = `For just <span>${dataItem.currency + data[0]}</span> more, get <span>${data[1]} GB extra</span>, reducing your cost per
        ...
        text = `For just <span>${dataItem.currency + data[0]}</span> more, get unlimited data plan. Don’t worry and enjoy your trip!`;
        upgrade = `
        <div class="upsell-block__col">
          <b>Unlimited plan</b>
      ...
      <div class="upsell-block__wrapper" data-index="${index}">
        <div class="upsell-block__descr">
          ...
          <button type="button" class="upsell-block__arrow">${dataIcons.arrow}</button>
        </div>
        <button type="button" class="upsell-block__btn" data-specCheck="${dataUpgrade.specCheck}" data-option="${dataUpgrade.option}" data-productId="${dataUpgrade.productId}" data-vid="${dataUpgrade.vid}">Upgrade plan</button>
    ...
    initUpsellBlock() {
      const mainCartStyles = /* HTML */ `
        <style class="style_cart">
          ...
          .CartItemWrapper.loading .CartItem:after {
            ...
            width: 100%;
            height: 100%;
            background: rgba(255, 255, 255, 0.7) url("https://conversionratestore.github.io/projects/lemieux/img/loading.gif") no-repeat center / 35px !important;
          }
        </style>
      `;
      document.head.insertAdjacentHTML("beforeend", mainCartStyles);

      if (!$el("#sidebar-cart .CartItemWrapper") || getLocalStorage("dataCollections") == null) return;
      ...
      $$el("#sidebar-cart .CartItemWrapper .CartItem").forEach((el, index) => {
        ...
        const vidEl = el.querySelector(".CartItem__Title a").href.split("variant=")[1];
        ...
        for (const key in dataCollections) {
          if (dataCollections[key]) {
            for (let i = 0; i < dataCollections[key].length; i++) {
              if (dataCollections[key][i].vid == vidEl && dataCollections[key][i].priceGB == priceEl) {
                ...
                if (i != 0) {
                  ...
                  let html = this.upsellBlockHtml([costDifference, costGb, percentageDifference], itemPrevDataCollections, itemDataCollections, index);

                  el.insertAdjacentHTML("afterend", html);
                } else {
                  if (!window.location.origin.includes("simsdirect")) return;

                  const title = el.querySelector(".CartItem__Title a").innerHTML.toLowerCase();

                  let keyUnlimitedData = title.includes("uk") || title.includes("turkey") ? "UK" : title.includes("japan") ? "Japan" : title.includes("China") ? "China" : title.includes("South Korea") ? "Korea" : title.includes("USA") ? "USA" : "Asia";

                  console.dir(title);
                  let arr = unlimitedData[keyUnlimitedData];
                  for (let k = 0; k < arr.length; k++) {
                    if (arr[k]["option-0"].toLowerCase().split(" days")[0] == itemDataCollections.days) {
                      console.dir(arr[k].price);
                      console.dir(itemDataCollections.priceGB);
                      console.dir(arr[k].price - itemDataCollections.priceGB);

                      let html = this.upsellBlockHtml([arr[k].price - itemDataCollections.priceGB], arr[k], itemDataCollections, index);
        ...
        if (!el.parentElement.querySelector(".upsell-block__wrapper")) return;
        let eventName = el.parentElement.querySelector(".upsell-block__wrapper").innerText.includes("unlimited") ? "exp_aov_improv_vis_cartunlimit_elem" : "exp_aov_improv_vis_cartlimit_elem";

        let eventLoc = el.parentElement.querySelector(".upsell-block__wrapper").innerText.includes("unlimited") ? "Unlimited" : "Limited";

        checkFocusTime(`.upsell-block__wrapper[data-index='${index}']`, eventName, `${el.querySelector(".CartItem__Title a").innerHTML} - Element view`, `Cart Don’t run out of data overseas! ${eventLoc} plan`);
      });
    ...
    onClickBtnUpsell() {
      $$el(".upsell-block__btn").forEach((el) => {
        ...
        el.addEventListener("click", (e) => {
          ...
          let line = el.closest(".CartItemWrapper").querySelector(".CartItem__Info .CartItem__Remove").dataset.line;
            ...
            .then((dataR) => {
              fetch("/cart/add.js", {
                ...
                .then((data) => {
                  ...
                  this.addUpdatedProduct(data, el.closest(".CartItemWrapper"), line);
      ...
      $$el(".crs_qty .QuantitySelector__Button").forEach((el) => {
        el.addEventListener("click", (e) => {
          ...
          el.closest(".CartItem__Actions").querySelector(`.QuantitySelector .quantity-dropdown li[value="${input.value}"] a`).click();
        });
      });
    }
    ...
    addUpdatedProduct(data, selector, line) {
      console.dir(data);
      const productHTML = `
      <div class="CartItem esim_show" data-price="${data.price}" data-handle="${data.handle}">
          ...
          <div class="CartItem__Info">
              ...
              <div class="CartItem__Meta Heading Text--subdued">
                  ...
                  <div class="CartItem__PriceList" data-disccode="">
                      ...
                      <span class="CartItem__Price Price">${this.currency}${addDotIfNeeded(data.price).split(".00")[0]}</span>
      ...
      $$el("#sidebar-cart .CartItemWrapper").forEach((el) => {
        ...
        qtySelector.insertAdjacentHTML(
          ...
          <div class="crs_qty">
            ...
            <input type="text" class="QuantitySelector__CurrentQuantity" pattern="[0-9]*" value="${qtySelector.querySelector("select").value}" aria-label="Quantity">
