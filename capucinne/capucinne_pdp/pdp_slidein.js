...
(function () {
      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          if (entry.target.classList.contains("lav-point__wrap")) {
            isElementInViewport(entry.target, ["new_payments_14_day_visibility", "14 day free block visibility", "Element visibility", "Product Information"]);
          }

          if (entry.target.classList.contains("lav-klarna")) {
            isElementInViewport(entry.target, ["new_payments_cart_clarna_vis", "Klarn block", "Element visibility", "Cart sidebar"]);
          }

          // TODO
          if (entry.target.classList.contains("needsclick")) {
            // console.log('ffff');
            isElementInViewport(entry.target.closest(".needsclick"), ["new_payments_pp_get_50_off", "Visibility", "Popup", "Popup 50$ off"], 3, () => {
          ...
          if (entry.target.classList.contains("lav-earn")) {
            if (entry.target.closest(".product-single__meta")) {
              isElementInViewport(entry.target, ["new_payments_free_earrings_vis", "Free earrings banner", "Element visibility", "Product Information"]);
            } else if (entry.target.closest(".drawer__inner")) {
              isElementInViewport(entry.target, ["new_payments_cart_free_earrings_vis", "Free earrings banner", "Element visibility", "Cart sidebar"]);
            }
          }

          if (entry.target.classList.contains("lav-benefits") && entry.target.closest(".product-single__form")) {
            isElementInViewport(entry.target, ["new_payments_rush_free_shipping1", "Free shipping (add to cart)", "Element visibility", "Product Information"]);
          }
          if (entry.target.classList.contains("lav-extend")) {
            isElementInViewport(entry.target, ["new_payments_extend_vis", "Extend block", "Element visibility", "Product Information"]);
          }
          if (entry.target.classList.contains("lav-shipping__rush")) {
            isElementInViewport(entry.target, ["new_payments_rush_order_vis", "Rush order is activated", "Element visibility", "Product Information"]);
          }
          if (entry.target.classList.contains("lav-summary")) {
            isElementInViewport(entry.target, ["new_payments_cart_subtotal_vis", "Subtotal block", "Element visibility", "Cart sidebar"]);
          }

          if (entry.target.classList.contains("drawer__header")) {
            isElementInViewport(entry.target, ["new_payments_cart_free_shipping", "Free shipping", "Element visibility", "Cart sidebar"]);
          }

          if (entry.target.classList.contains("lav-benefits") && entry.target.closest(".lav-shipping")) {
            isElementInViewport(entry.target, ["new_payments_rush_free_shipping2", "Free shipping (faq)", "Element visibility", "Product Information"]);
          }
        }
      });
    }, observerOptions);
  ...
  function init() {
    ...
    waitFor(
      () => $el('[href="#reviews"]'),
      () => {
        $el('[href="#reviews"]').closest(".product-block")?.classList.add("lav-reviews");

        $el(".lav-reviews > div span").innerText = "Over " + $el(".lav-reviews > div span").innerText.match(/\d+\s\w+/)[0];

        $el("[data-product-blocks]").insertAdjacentElement("afterbegin", $el('[href="#reviews"]').closest(".product-block"));
      }
    );
  ...
  `;
    ...
    waitFor(
      ...
      () => {
        $el(".product-block--sales-point").insertAdjacentHTML("beforebegin", points);
    ...
    waitFor(
      ...
      () => {
        ...
        $el(".product-single__form .add-to-cart").insertAdjacentHTML("afterbegin", `<img src='${exp.dir}/img/bag.svg' />`);

        $el(".product-single__form .add-to-cart").addEventListener("click", () => {
          if (!isAddCart) {
            pushDataLayer("new_payments_add_to_cart_new", "Add to cart new", "Button", "Product Information");
          }
        });

        $el(".product-single__form .add-to-cart").insertAdjacentHTML("afterend", benefits);
      }
    );
  ...
  function handleDocumentClick() {
    document.addEventListener("click", (e) => {
      ...
      if (target.closest(".klaviyo-close-form")) {
        pushDataLayer("new_payments_pp_get_50_off_close", `Close. ${target.closest(".klaviyo-close-form")?.nextElementSibling?.querySelector('[data-testid="form-component"]')?.innerText?.trim()}`, "Button", "Popup 50$ off");
      }

      if (target.closest(".needsclick")) {
        if (target.tagName === "INPUT" && target.placeholder === "Enter your email address") {
          pushDataLayer("new_payments_pp_get_50_off_email", "Email", "Input", "Popup 50$ off");
        }

        if (target.tagName === "BUTTON" && target.innerText === "UNLOCK MY DEAL" && klaviyoStep < 3) {
          pushDataLayer("new_payments_pp_get_50_off_unlock", `Unlock my deal. ${klaviyoStep === 1 ? "Step 1" : "Step 2"}`, "Button", "Popup 50$ off");
        ...
        if (target.tagName === "BUTTON" && (target.innerText === "No thanks I’ll pay full price" || target.innerText === `No thanks, I'll pay the full price`) && klaviyoStep < 3) {
          pushDataLayer("new_payments_pp_get_50_off_no", `No, thanks. ${klaviyoStep === 1 ? "Step 1" : "Step 2"}`, "Button", "Popup 50$ off");
        }

        if (target.closest('[component="[object Object]"]')?.querySelector("svg path")?.getAttribute("d") === `M11.1597 18.9917L6.66651 14.4983C5.99844 13.8687 4.95494 13.8706 4.28909 14.5025C3.59908 15.1574 3.56725 16.2465 4.21784 16.9405L9.72916 23.085C10.5199 23.9286 11.862 23.9189 12.6405 23.064L25.9625 8.9336C26.5713 8.26509 26.5411 7.23449 25.8943 6.60272C25.2293 5.95322 24.1631 5.96785 23.5162 6.63534L11.1597 18.9917Z`) {
          sessionStorage.setItem("lav-discount", "yes");

          pushDataLayer("new_payments_pp_get_50_off_copy", "Copy discount code", "Button", "Popup 50$ off");
        }

        if (target.tagName === "BUTTON" && target.innerText === "CONTINUE SHOPPING") {
          sessionStorage.setItem("lav-discount", "yes");

          pushDataLayer("new_payments_pp_get_50_off_continue", "Continue shopping", "Button", "Popup 50$ off");
        }
      }
    });
  }
  ...
  function updateData() {
    ...
    const subtotal = parseInt($el("[data-cart-subtotal]")?.getAttribute("data-cart-subtotal")) / 100;
    ...
    if (parseInt(localStorage.getItem("subtotal")) === parseInt(subtotal)) return false;
  ...
  function handleKlarna() {
    if ($el('#CartDrawerForm [part="osm-container"]') || !$el("klarna-placement div")) return false;
    if ($el("klarna-placement div").style.display === "none") return false;
    const original = $el("klarna-placement div")?.shadowRoot?.querySelector("div");

    original.querySelector(".container").style = "border: 0;padding: 3px 0;";
    original.querySelector(".text-wrapper").style = "margin-left: 9px;";
    original.querySelector('.text-wrapper .text[part="osm-message"]').style = "color: #565656";
    ...
    $el("klarna-placement div")
      ...
      .addEventListener("click", () => {
        if (!isKlarnaClick) {
          pushDataLayer("new_payments_klarna_link", "Klarna learn more", "Link", "Product Information");
        }

        setTimeout(() => {
          $el("klarna-osm-interstitial")
            ?.shadowRoot.querySelector("#learn-more-dialog-payment_calculator-light__footer-button-wrapper")
            ?.addEventListener("click", () => {
              pushDataLayer("new_payments_klarna_go_it", "Go it", "Button", "Klarna popup");
            });

          $el("klarna-osm-interstitial")
            ?.shadowRoot.querySelector("#learn-more-dialog-payment_calculator-light__nav-bar__right-icon")
            ?.addEventListener("click", () => {
              pushDataLayer("new_payments_klarna_close", "Close", "Button", "Klarna popup");
            });
        }, 1000);
      });
    ...
    clone.querySelector('[part="osm-cta"]').addEventListener("click", (e) => {
      e.preventDefault();
      pushDataLayer("new_payments_cart_clarna_learn", "Klarna learn more", "Link", "Cart sidebar");
      isKlarnaClick = true;
      $el("klarna-placement div")?.shadowRoot?.querySelector('[part="osm-cta"]').click();
      isKlarnaClick = false;
    });
  ...
  function handleExtend() {
    if ($el(".lav-extend") || !$el(".extend-offer iframe")?.contentWindow?.document?.querySelector(".btn-offer .term-length")?.textContent.trim() || !$el(".product-single__form .add-to-cart + .lav-benefits")) return false;
    ...
    $el(".product-single__form .add-to-cart + .lav-benefits").insertAdjacentHTML("afterend", extend);
    ...
    for (let item of innerEl.querySelectorAll(".btn-offer")) {
      ...
      el.innerHTML = `
        <div class='lav-extend__item-caption'>${item.querySelector(".term-length").textContent}</div>
        <div class='lav-extend__item-price'>${item.querySelector(".plan-price").textContent}</div>
      `;

      document.querySelector(".lav-extend__body").insertAdjacentElement("beforeend", el);
    ...
    $el(".lav-extend__covered").addEventListener("click", () => {
      pushDataLayer("new_payments_extend_what_is_covered", "What is covered", "Link", "Product Information");
      innerEl.querySelector(".info-link").click();
    });
  }

  function handleItemsSummary() {
    for (let item of $$el(".lav-summary__item:not(.lav-summary__item-subtotal)")) {
      item.remove();
    }

    for (let item of $$el(".cart__items .cart__item")) {
      const el = `
        <div class="lav-summary__item ${item.querySelector(".cart__item-name").innerText.trim() === "Free Gift" ? "lav-summary__item-gift" : ""}">
          <div class="lav-summary__caption">
          ${item.querySelector(".cart__item-name").innerText}
          ${item.querySelector(".cart__item-name").innerText.trim() === "Free Gift" ? `(${item.querySelector(".cart__item--variants").innerText.replace("Product:", "").trim()})` : ""}
          </div>
          <div class="lav-summary__value">
          ${item.querySelector("small.cart__discount") ? item.querySelector(".cart__price.cart__discount span").innerText : item.querySelector(".cart__price").innerText} 
          ${item.querySelector(".js-qty__num")?.value > 1 ? `(X${item.querySelector(".js-qty__num")?.value})` : ""}
      </div>
        </div>
      `;
  ...
  function handleHand() {
    if (!$el(".product-single__meta .tangiblee-cta-wrapper") || $el(".product-single__meta .tangiblee-cta")?.style.display === "none") {
      if ($el(".product-single__meta .tangiblee-cta")?.style.display === "none") {
        $el(".product-single__meta .tangiblee-cta-wrapper")?.classList.add("lav-hide");
      }
      return false;
    }

    $el(".product-single__meta .tangiblee-cta-wrapper").classList.add("lav-hand");

    $el(".lav-hand").addEventListener("click", () => {
      pushDataLayer("new_payments_try_it_on_your_hand", "Try it on your hand", "Button", "Gallery");
    });

    $el(".product__main-photos").insertAdjacentElement("afterbegin", $el(".product-single__meta .tangiblee-cta-wrapper"));
  ...
  function handleDiscount() {
    if (!$el('.needsclick[aria-label="Open Form"]')) {
      ...
      if (sessionStorage.getItem("lav-discount") === "yes") {
        if (!$el(".product-block--price .lav-discount")) {
          $el("[data-product-price]").insertAdjacentHTML("beforeend", appliedDiscount);
        }

        if (!$el(".lav-sticky .lav-discount")) {
          $el(".lav-sticky__price").insertAdjacentHTML("beforeend", appliedDiscount);
        }
      }
    ...
    if (!$el(".product-block--price .lav-discount")) {
      ...
      $el(".product-block--price .lav-discount").addEventListener("click", () => {
        if ($el('.needsclick[aria-label="Open Form"]')) {
          pushDataLayer("new_payments_pdp_get_your_50_off", "Get your 50$ off", "Link", "Product Information");
  ...
  function handleEarn(subtotal) {
    ...
    if (!$el("[data-product-blocks] .lav-earn")) {
      $el(".the4-toolkit-wishlist")?.insertAdjacentHTML("beforebegin", earnStart);
    }
    ...
    if (subtotal >= 5000 && !$el("[data-product-blocks] .lav-earn-5000")) {
      ...
      $el(".the4-toolkit-wishlist")?.insertAdjacentHTML("beforebegin", earnGift2);
      $el("[data-product-blocks] .lav-earn a")?.addEventListener("click", () => {
        pushDataLayer("new_payments_learn_more_brecelet", "Learn more (earrings)", "Link", "Product Information");
      });
    } else if (subtotal < 5000 && subtotal >= 1500 && !$el("[data-product-blocks] .lav-earn-1500")) {
      $el("[data-product-blocks] .lav-earn")?.remove();
      $el(".the4-toolkit-wishlist")?.insertAdjacentHTML("beforebegin", earnGift1);
      $el("[data-product-blocks] .lav-earn a")?.addEventListener("click", () => {
        pushDataLayer("new_payments_learn_more_earrings", "Learn more (earrings)", "Link", "Product Information");
      });
    } else if (subtotal < 1500 && !$el("[data-product-blocks] .lav-earn-start")) {
      $el("[data-product-blocks] .lav-earn")?.remove();
      $el(".the4-toolkit-wishlist")?.insertAdjacentHTML("beforebegin", earnStart);
    }
  }
  ...
  function handleShipping(benefits) {
    const el = Array.from($$el(".product-block--tab")).find((item) => {
      return item.querySelector(".label").innerText.includes("Shipping and Production");
    });
    ...
    el.querySelector(".label").innerHTML = el.querySelector(".label").innerHTML.replace("Shipping and Production", "Shipping, returns and exechnges");
  ...
  function addSticky() {
    ...
    if ($el('[name="properties[Ring size]"]')) {
      ...
      $el(".extend-offer").insertAdjacentHTML("afterend", '<div class="lav-select_size">Add to cart</div>');
  ...
  function handleSlideIn(benefits) {
    ...
    $el(".lav-notes", el).addEventListener("click", function (e) {
      e.preventDefault();
      e.stopPropagation();
      pushDataLayer("new_payments_cart_leave_order_notes", "Leave order notes", "Link", "Cart sidebar");
    ...
    $el(".cart__checkout-wrapper button", el).addEventListener("click", () => {
      pushDataLayer("new_payments_cart_checkout", "Checkout", "Button", "Cart sidebar");
    });
    ...
    if ($el(".lav-paypal")) {
      ...
      if ($el('.lav-paypal [data-testid="grid-cell"]')) {
        $el('.lav-paypal [data-testid="grid-cell"]').style.flexGrow = "1!important";
      } else {
        setTimeout(() => {
          if ($el('.lav-paypal [data-testid="grid-cell"]')) {
            $el('.lav-paypal [data-testid="grid-cell"]').style.flexGrow = "1!important";
      ...
      $el(".lav-paypal").addEventListener("click", () => {
        pushDataLayer("new_payments_cart_paypal", "PayPal", "Button", "Cart sidebar");
      });
    }
  }
  ...
  function handleItems(el) {
    Array.from($$el(".cart__item", el)).forEach((pr) => {
      if ($el('.needsclick[aria-label="Open Form"]') && !pr.querySelector(".lav-discount") && !pr.querySelector(".cart__item-name").innerText.toLowerCase().includes("gift") && !pr.querySelector(".cart__item-name").innerText.toLowerCase().includes("extend protection")) {
        if (!pr.querySelector(".cart__item-name").innerText.toLowerCase().includes("engraving") && !pr.querySelector(".cart__item-name").innerText.toLowerCase().includes("rush")) {
          pr.querySelector(".cart__price").insertAdjacentHTML("afterend", getDiscount);
        }

        pr.querySelector(".lav-discount")?.addEventListener("click", function (e) {
          if ($el('.needsclick[aria-label="Open Form"]')) {
            pushDataLayer("new_payments_cart_get_your_50_off", "Get your 50$ off", "Link", "Cart sidebar");
      ...
      if (!$el('.needsclick[aria-label="Open Form"]') && sessionStorage.getItem("lav-discount") === "yes" && !pr.querySelector(".lav-discount.lav-discount_applied") && !pr.querySelector(".cart__item-name").innerText.toLowerCase().includes("gift") && !pr.querySelector(".cart__item-name").innerText.toLowerCase().includes("extend protection")) {
        pr.querySelector("small.cart__discount").insertAdjacentHTML("afterend", appliedDiscount);
        pr.querySelector(".lav-discount__caption").innerText = `You save ${pr.querySelector("small.cart__discount span").innerText}`;
      }

      if (!pr.querySelector(".extend-simple-offer iframe") && pr.querySelector(".lav-p-extend")) {
        pr.querySelector(".lav-p-extend").remove();
      }

      if (pr.querySelector(".extend-simple-offer iframe") && !pr.querySelector(".lav-p-extend")) {
        pr.querySelector(".cart__item-details").insertAdjacentHTML("beforeend", productExtend);

        pr.querySelector(".lav-p-extend").addEventListener("click", function () {
          pushDataLayer("new_payments_cart_protect_your_gewellery", "Protect your gewellery", "Link", "Cart sidebar");
          pr.querySelector(".extend-simple-offer iframe").contentDocument.querySelector("button").click();
        });
      }

      if (!pr.querySelector(".js-qty__wrapper") || pr.querySelector(".lav-product__remove")) {
        return false;
      }

      pr.querySelector(".js-qty__wrapper").insertAdjacentHTML("beforeend", `<div class='lav-product__remove'>Remove</div>`);

      pr.querySelector(".lav-product__remove").addEventListener("click", function (e) {
        if (pr.querySelector(".js-qty__num")) {
          pushDataLayer("new_payments_cart_remove", "Remove", "Link", "Cart sidebar");
  ...
  function findGift() {
    ...
    if (gift) {
      ...
      $el('.drawer__scrollable [for="CartNoteDrawer"]').closest("div").insertAdjacentHTML("afterbegin", earnStart);
    }
  }
