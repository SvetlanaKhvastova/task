let socialProof = setInterval(() => {
    if (document.querySelector('.reviews_box')) {
        ...
        let arrReviewsPartOne = {
            ...
            'Tammy Ochoa, US': [5, `Our puppy became an emotional support dog`, `“I have a very high spirited 11 week old German Shepherd that is super smart and learns very quickly. I want her to be an emotional support dog for my husband who is a combat veteran and has PTSD and some health problems. This program shows another way of training to accomplish that calm, well-behaved dog that is desired.“`],
        }
        let arrReviewsPartTwo = {
            'Dorothy Alspaugh, US': [5, 'Wish I had seen this 3 dogs ago', `“The course gives you the best information about how to manage and train a dog not to bark, not to jump up on people and how to get along with other dogs! It is a must learning tool for anyone who owns a dog. Wish I had seen this 3 dogs ago.“`],
        ...
        function setReviews(name, star, title, text) {
            return ` <li>
        ...
        if (document.querySelector('.reviews_wrap')) {
            if (document.querySelector('.reviews_wrap.part_one')) {
                for (let key in arrReviewsPartOne) {
                    document.querySelector(".reviews_wrap.part_one").insertAdjacentHTML("beforeend", setReviews(key, arrReviewsPartOne[key][0], arrReviewsPartOne[key][1], arrReviewsPartOne[key][2]))
                }
            }
            if (document.querySelector('.reviews_wrap.part_two')) {
                for (let key in arrReviewsPartTwo) {
                    document.querySelector(".reviews_wrap.part_two").insertAdjacentHTML("beforeend", setReviews(key, arrReviewsPartTwo[key][0], arrReviewsPartTwo[key][1], arrReviewsPartTwo[key][2]))
                }
            }
