...
class NewAstrologersScreen {
  ...
  init() {
    ...
    clarityInterval('exp_astro_screen', 'variant_2')
    this.observeMain()
  ...
  renderNewAstrologersScreenSection() {
    waitForElement('main').then(element => {
      ...
      if (!$el('.new_astrologers_screen')) {
        mainElement.insertAdjacentHTML('beforebegin', newAstrologersScreenVerC)
      }
    })
    ...
    waitForElement('.new_astrologers_screen').then(element => {
      const mainElement = element as HTMLElement
      pushData('exp_new_astro_visibility_02', 'Screen view', 'View', 'palm readers')
    })
  }
    ...
    waitForElement('[data-nextBtn]').then(element => {
      ...
      buttons.forEach(e => {
        e.addEventListener('click', () => {
          if (e.classList.contains('new_continue_btn')) {
            pushData('exp_new_astro_button_03', `Continue - ${this.storage}`, 'Click', 'palm readers')
          } else {
            pushData('exp_new_astro_button_05', 'Chose later', 'Click', 'palm readers')
          }
  ...
  clickNewBtnBackHandler() {
    waitForElement('.new_btn_back').then(element => {
      ...
      backButton.addEventListener('click', () => {
        pushData('exp_new_astro_button_04', 'Back', 'Click', 'palm readers')
        if (localStorage.getItem('newAstrologersScreen')) {
