...
import { newBtnSkipVerB } from './blocks'
...
class ProvideNewAlternatives {
  ...
  addClassToBody() {
    waitForElement('[alt="arrow-top"]').then(i => {
      ...
      pushData('exp_hyp5_visibility_01', 'Screen view', 'View', 'Try hint for 1 week')
    })
  }
  ...
  renderNewBtnSkip() {
    waitForElement('.crs_subscriptionPlan [alt="arrow-top"]').then(i => {
      waitForElement('.bg-gray-100').then(i => {
        ...
        const price = $$el('.bg-gray-100')[0]?.textContent || ''

        if (!$el('.new_btn_skip')) {
          сontainerElement.insertAdjacentHTML('beforeend', newBtnSkipVerB(price))
        }
