...
function handleScroll(e) {
  let verticalScrollPercentage = (e.target.scrollTop / (e.target.scrollHeight - e.target.clientHeight)) * 100;
  this.pushDataLayer(["exp_exit_int_popup_scro_almos_vert", `percent_scrolled: ${verticalScrollPercentage}`, "Scroll ", "Popup It’s almost yours! Products group"]);
}
...
class ExitIntentPopup {
  ...
  init() {
    const globalMutation = new MutationObserver((mutations) => {
      ...
      if (this.checkPageUrl() === "checkout" && clickAddToCart == false && this.device == "mobile") {
    ...
    if (!sessionStorage.getItem("popupShown") && this.isLongInactivityInBag == false) {
      ...
      function resetIdleTimer() {
        ...
        idleTimer = setTimeout(() => {
          if ((new ExitIntentPopup(device).checkPageUrl() === "bag" || new ExitIntentPopup(device).checkPageUrl() === "checkout") && localStorage.getItem("crs_cart") && !sessionStorage.getItem("popupShown")) {
  ...
  setExitIntentPopup() {
    if (this.device == "desktop") {
      ...
      document.body.addEventListener(
        ...
        function () {
          if (x < 50 || y < 50 || x > window.innerWidth - 50 || y > window.innerHeight - 50) {
            if (localStorage.getItem("crs_cart") && clickAddToCart == false && !sessionStorage.getItem("popupShown")) {
              new ExitIntentPopup(device).showPopup();
            }
          }
        },
      ...
      let scrollSpeed = () => {
        ...
        if (currentSpeed > speedValue && localStorage.getItem("crs_cart") && window.scrollY != 0 && clickAddToCart == false && !sessionStorage.getItem("popupShown")) {
  ...
  addPopup() {
    ...
    $el(".crs_popup_close").addEventListener("click", () => {
      ...
      const page = $$el('[aria-label="Breadcrumb"] li').length >= 3 && this.checkPageUrl() == "shop" ? "PDP" : this.checkPageUrl();
      this.pushDataLayer(["exp_exit_int_popup_vis_almos_focu", `${startTime} - ${page}`, "Visibility ", "Popup It’s almost yours!"]);
      this.pushDataLayer(["exp_exit_int_popup_vis_almcompl_focu", startTime, "Visibility ", "Popup It’s almost yours! Complete my order now"]);
      this.pushDataLayer(["exp_exit_int_popup_but_almos_close", "Close", "Button", "Popup It’s almost yours!"]);
    });
    ...
    $el(".crs_popup_complete").addEventListener("click", (e) => {
      ...
      this.pushDataLayer(["exp_exit_int_popup_but_almos_compl", "Complete my order now", "Button", "Popup It’s almost yours!"]);
    ...
    if ($el('[aria-label="Bag"]') && $el(`[data-testid="${testid}"]`) && $el(`[data-testid="${testid}"]`).innerText == "" && localStorage.getItem("crs_cart")) {
  ...
  addDataCartInStorage() {
    if ($el("#main .layout-container h1")) {
      $$el("button.tracking-widest").forEach((item) => {
        if (item.innerText.toLowerCase().includes("add to bag") && usedDefaultLog == false) {
          usedDefaultLog = true;
          ...
          function checkData(val) {
            if (val.includes("item_id")) {
              try {
                ...
                let title = parsedData[0].split('"item_name":')[1].split(",")[0].split('"').join("").trim();
                console.log(title);

                if ($el("#main .layout-container h1").innerText.includes(title)) {
                  let metal = $el("label.sr-only") ? $el("label.sr-only").nextElementSibling.querySelector('.cursor-pointer[data-headlessui-state="checked"]').innerText.replace("Pure ", "") : "";

                  let weight = $el("label.sr-only").closest(".flex-col").querySelector(".flex-col .text-p").innerText.replace("24K ", "");
                  let image = $el("#carousel-slide-0 > img") ? $el("#carousel-slide-0 > img").src : $el(".sticky img.object-cover") ? $el(".sticky img.object-cover").src : "";
                  ...
                  dataCart.push({
                    ...
                    low_stock: +parsedData[0].split('"low_stock":')[1].split(",")[0].trim(),
                    image: image,
                    item_id: parsedData[0].split('"item_id":')[1].split(",")[0].trim(),
                    item_variant: parsedData[0].split('"item_variant": ')[1].split(",")[0].split('"').join("").trim(),
                    metal: metal + ", " + weight,
                  });

                  if (localStorage.getItem("crs_cart") != "undefined" && localStorage.getItem("crs_cart") != null && localStorage.getItem("crs_cart") != "") {
                    dataCart = [...dataCart, ...JSON.parse(localStorage.getItem("crs_cart"))];
                  }
                  dataCart = dataCart.filter((thing, index, self) => index === self.findIndex((t) => t.item_name === thing.item_name && t.item_variant === thing.item_variant));
    ...
    if (localStorage.getItem("crs_cart")) {
      $$el(".flex-col > .justify-start.gap-4 > div:last-child > .text-black > button").forEach((el) => {
        el.addEventListener("click", (e) => {
          ...
          let itemName = el.closest(".relative").querySelectorAll("h5")[0].innerText.toLowerCase();
          ...
          for (let i = 0; i < dataCart.length; i++) {
            ...
            if (dataCart[i]["item_name"].toLowerCase().includes(newItemName) && dataCart[i]["metal"].toLowerCase().includes(itemMetal)) {
              dataCart.splice(i, 1);
      ...
      $el(".crs_popup .crs_list").insertAdjacentHTML(
        ...
        <li data-id="${dataCart[i].item_id}">
          ...
          <div>
              ...
              <p class="crs_popup_left_stock ${dataCart.length > 1 ? "" : "justify-center"} items-center ${dataCart[i].low_stock <= 4 && typeof dataCart[i].low_stock != "object" ? "flex" : ""}" >
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                ...
                <span>${madeForYou != "" ? madeForYou : `Only ` + dataCart[i].low_stock + `pcs left in stock`}
                </span> 
              </p>
          </div>
      </li>`
...
https://calendly.com/treble-health-audiologists/tinnitus-relief-discovery?embed_domain=treblehealth.com&embed_type=Inline&hide_event_type_details=1&hide_gdpr_banner=1&primary_color=4622da&month=2023-11&utm_content=funnel_1&utm_id=1261762004.1699950023

https://calendly.com/treble-health-audiologists/treble-health-consultation?embed_domain=treblehealth.com&amp;embed_type=Inline&amp;hide_event_type_details=1&amp;hide_gdpr_banner=1&amp;primary_color=4622da&amp;utm_term=1261762004.1699950023&utm_content=funnel_1