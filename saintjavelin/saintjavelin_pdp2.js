...
function drawSelectList() {
  ...
  document.querySelectorAll('fieldset[name="Size"] input').forEach((item) => {
    if (item.value !== "default") {
      query(".select_wrapper ul").insertAdjacentHTML(
        "beforeend",
        `<li data-value="${item.value}" data-disabled="${item.classList.contains("disabled")}" data-checked="${item.checked}">${item.value}</li>`
      )
      query(".mobile_size ul").insertAdjacentHTML(
        "beforeend",
        `<li data-value="${item.value}" data-disabled="${item.classList.contains("disabled")}" data-checked="${item.checked}">${item.value}</li>`
      )
    }
